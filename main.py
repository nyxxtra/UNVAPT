...
from ports.ssh_22 import scan_port_22
from ports.telnet_23 import scan_port_23
...
def show_custom_window(title, message, target_ip):
	...
	# Check if port 22 is open and add a button for port 22 scan
    if "22/tcp" in message and "open" in message:
        port_22_button = tk.Button(window, text="Scan Port 22", command=lambda: scan_port_22_window(target_ip), font=("Times New Roman", 14), width=15)
        port_22_button.pack(pady=5)

	# Check if port 23 is open and add a button for port 23 scan
    if "23/tcp" in message and "open" in message:
        port_23_button = tk.Button(window, text="Scan Port 23", command=lambda: scan_port_23_window(target_ip), font=("Times New Roman", 14), width=15)
        port_23_button.pack(pady=5)
...
# Function to open a new window for port 21 scan
...
# Function to open a new window for port 22 scan
def scan_port_22_window(target_ip):
    # Perform a specific scan for port 22 (SSH)
    port_22_output = scan_port_22(target_ip)
    if port_22_output:
        show_custom_window("Port 22 Scan Results", port_22_output, target_ip)

# Function to open a new window for port 23 scan
def scan_port_23_window(target_ip):
    # Perform a specific scan for port 23 (telnet)
    port_23_output = scan_port_23(target_ip)
    if port_23_output:
        show_custom_window("Port 23 Scan Results", port_23_output, target_ip)
